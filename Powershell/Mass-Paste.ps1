...
For($i=0; $i -lt 201; $i++){
  ...
  Add-Content -Path $file -Value "    <set name=&PCRContacts& alias=&MF$($ContactNumber)&>
      <field name=&ContactName&>MF$($ContactNumber)</field>
      ...
      <collection name=&DigitalCalls&>
        <set name=&DigitalCalls& index=&0& key=&Private Call&>
          <field name=&DU_CALLALIAS&>MF$($ContactNumber)</field>
